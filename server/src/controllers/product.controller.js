...
const create = async (req, res) => {
  try {
    const { name, description, cost_per_kg, category_id, is_featured, sort_order } = req.body;
    let image_url = null, image_public_id = null;

    if (req.file) {
      const result = await cloudinary.uploader.upload(req.file.path, { folder: 'sunday-chicken/products', quality: 'auto', fetch_format: 'auto' });
      image_url       = result.secure_url;
      image_public_id = result.public_id;
    }

    const { rows } = await query(
      `INSERT INTO products (name, description, cost_per_kg, category_id, image_url, image_public_id, is_featured, sort_order)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
      [name, description, cost_per_kg, category_id || null, image_url, image_public_id, is_featured || false, sort_order || 0]
    );
...
const update = async (req, res) => {
  try {
    const { name, description, category_id, is_featured, is_available, sort_order } = req.body;
    let extra = '', extraParams = [];
    if (req.file) {
      const result = await cloudinary.uploader.upload(req.file.path, { folder: 'sunday-chicken/products' });
      extra = ', image_url=$6, image_public_id=$7';
      extraParams = [result.secure_url, result.public_id];
    }
    const { rows } = await query(
      `UPDATE products SET name=$1, description=$2, category_id=$3, is_featured=$4, is_available=$5, sort_order=${extraParams.length ? '$8' : '$6'}${extra}
       WHERE id=${extraParams.length ? '$9' : '$7'} RETURNING *`,
      [name, description, category_id || null, is_featured, is_available, sort_order, ...extraParams, req.params.id]
    );
    if (!rows[0]) return errorResponse(res, 'Product not found', 404);
...
const updateCost = async (req, res) => {
  try {
    ...
    if (!cost || isNaN(cost) || Number(cost) <= 0)
      return errorResponse(res, 'Invalid cost value', 422);
    const { rows } = await query(
...
const remove = async (req, res) => {
  try {
    const { rows } = await query('DELETE FROM products WHERE id=$1 RETURNING image_public_id', [req.params.id]);
    if (!rows[0]) return errorResponse(res, 'Product not found', 404);
